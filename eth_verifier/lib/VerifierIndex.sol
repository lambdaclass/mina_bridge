    ...
    function verifier_digest(VerifierIndex storage index) public returns (Base.FE) {
        ...
        if (index.is_lookup_index_set) {
            ...
            if (l_index.is_runtime_tables_selector_set) {
                index.sponge.absorb_g(
                    l_index.runtime_tables_selector.unshifted
                );
            }
    ...
    function get_column_commitment(
        ...
        if (variant == ColumnVariant.Witness) {
            ...
            if (pattern == LookupPattern.Xor) {
                ...
                return
                    verifier_index.lookup_index.lookup_selectors.xor.unshifted[
                        0
                    ];
            }
            if (pattern == LookupPattern.Lookup) {
                if (
                    !verifier_index.lookup_index.lookup_selectors.is_lookup_set
                ) {
                    revert MissingLookupColumnCommitment(pattern);
                }
                return
            ...
            if (pattern == LookupPattern.ForeignFieldMul) {
                if (
                    !verifier_index.lookup_index.lookup_selectors.is_ffmul_set
                ) {
                    revert MissingLookupColumnCommitment(pattern);
                }
                return
            ...
            return
                verifier_index.lookup_index.runtime_tables_selector.unshifted[
                    0
                ];
            ...
            if (gate == GateType.Generic) {
                return verifier_index.generic_comm.unshifted[0];
            } else if (gate == GateType.CompleteAdd) {
                return verifier_index.complete_add_comm.unshifted[0];
            } else if (gate == GateType.VarBaseMul) {
                return verifier_index.mul_comm.unshifted[0];
            } else if (gate == GateType.EndoMul) {
                return verifier_index.emul_comm.unshifted[0];
            } else if (gate == GateType.EndoMulScalar) {
                return verifier_index.endomul_scalar_comm.unshifted[0];
            } else if (gate == GateType.Poseidon) {
                return verifier_index.psm_comm.unshifted[0];
