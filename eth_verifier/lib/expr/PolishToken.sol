...
library PolishTokenEvaluation {
    ...
    function evaluate(
        ...
        for (uint256 i = 0; i < toks.length; i++) {
            ...
            if (v == PolishTokenVariant.Mds) {
                PolishTokenMds memory pos = abi.decode(
                    v_data,
                    (PolishTokenMds)
                );
            ...
            if (
                v == PolishTokenVariant.VanishesOnZeroKnowledgeAndPreviousRows
            ) {
                stack[stack_next] = Polynomial.eval_vanishes_on_last_n_rows(
                    domain_gen,
                    domain_size,
                    c.zk_rows + 1,
                    pt
            ...
            if (v == PolishTokenVariant.UnnormalizedLagrangeBasis) {
                ...
                stack[stack_next] = unnormalized_lagrange_basis(
                    domain_gen,
                    vanishing_eval,
                    offset,
                    pt
    ...
    function unnormalized_lagrange_basis(
        ...
        int256 i,
        Scalar.FE pt
    ) internal view returns (Scalar.FE result) {
        Scalar.FE omega_i;
        if (i < 0) {
    ...
    function evaluate_vanishing_polynomial(
        Scalar.FE domain_gen,
        uint256 domain_size,
        Scalar.FE tau
