...
library MsgPk {
    ...
    function deser_prover_proof(Stream memory self, ProverProof storage prover_proof) external {
        ...
        if (is_runtime_lookup_table_selector_set) {
            prover_proof.evals.runtime_lookup_table_selector =
                deser_evals(all_evals_map, "runtime_lookup_table_selector");
        }
        ...
        bytes memory foreign_field_mul_lookup_selector =
            find_value_str(all_evals_map, "foreign_field_mul_lookup_selector");
        ...
        if (is_foreign_field_mul_lookup_selector_set) {
            prover_proof.evals.foreign_field_mul_lookup_selector =
                deser_evals(all_evals_map, "foreign_field_mul_lookup_selector");
        }
    ...
    function deser_pairing_urs(Stream memory self, PairingURS storage urs) public {
        ...
        EncodedMap memory verifier_urs_serialized = abi.decode(find_value_str(urs_map, "verifier_srs"), (EncodedMap));
        ...
        bytes memory verifier_urs_h_serialized = abi.decode(find_value_str(verifier_urs_serialized, "h"), (bytes));
    ...
    function deser_polishtoken(EncodedMap memory map) public view returns (PolishToken memory) {
        ...
        if (is_unit) {
            ...
            (bytes memory ulag_value, bool is_ulag) = find_value_or_fail(map, abi.encode("value"));
            if (is_ulag) {
