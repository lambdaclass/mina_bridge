...
fn generate_proof() {
    let (mut proof, public_input): (ProverProof<G1, KZGProof>, Vec<[u64; 4]>) =
        serde_json::from_str(&fs::read_to_string("./proof_with_public.json").unwrap()).unwrap();
    proof.commitments.t_comm.unshifted[0] =
        G1Affine::get_point_from_x(BaseField::from(3), true).unwrap();
